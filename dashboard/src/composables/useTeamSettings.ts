...
const renameCall = useCall<{ team_name: string }, { team: string; team_name: string }>({
...
export function useTeamSettings() {
  ...
  const isOwner = computed(
    () => session.teams.value.find((t) => t.name === activeTeam.value)?.owner === currentUser.value,
  )
  ...
  function rename(teamName: string) {
    return run(renameCall, { team: activeTeam.value!, team_name: teamName }, () => session.reload(), 'Team renamed.')
  }

  function transferOwnership(user: string) {
    return run(transferCall, { team: activeTeam.value!, user }, async () => {
  ...
  function deleteTeam() {
    return run(deleteCall, { team: activeTeam.value! }, async () => {
      await session.reload()
  ...
  function createTeam(teamName: string) {
    return run(createCall, { team_name: teamName }, async () => {
      await session.reload()
  ...
  return { isOwner, saving: computed(() => saving.value), rename, transferOwnership, deleteTeam, createTeam }
}
