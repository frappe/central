...
import { ListView, createListViewQuery, type ListViewColumn, type ListViewFilter } from '@/components/common/list-view'
...
const query = ref(createListViewQuery({ pageSize: 20, sort: { key: 'creation', direction: 'desc' } }))
...
const filters: ListViewFilter[] = [
  {
    key: 'status',
    label: 'Status',
    options: ['Pending', 'Accepted', 'Expired', 'Revoked', 'Declined'].map((v) => ({ label: v, value: v })),
  },
]

const columns = computed<ListViewColumn<InvitationRow>[]>(() => [
  { accessorKey: 'email', header: 'Email', meta: { cellClass: 'truncate font-medium' } },
  { id: 'role', header: 'Role', accessorFn: (i) => roleLabel(i.role), meta: { cellClass: 'truncate' } },
  {
    accessorKey: 'status',
    header: 'Status',
    enableSorting: false,
    cell: ({ row }) => h(Badge, { theme: invitationStatusTheme(row.original.status), label: row.original.status }),
  },
  { id: 'expires', header: 'Expires', accessorFn: (i) => formatDate(i.expires_on) || '-' },
  {
    id: 'actions',
    header: '',
    enableSorting: false,
  ...
  <div class="min-w-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
    ...
    >
      <template v-if="canManageMembers" #toolbar>
        <Button variant="solid" label="Invite" icon-left="lucide-user-plus" @click="inviteOpen = true" />
      </template>
    </ListView>
  </div>
