...
const PREFS: { key: string; label: string; hint: string }[] = [
  { key: 'notify_payment_failure', label: 'Payment failed', hint: 'A charge was declined.' },
  { key: 'notify_payment_retry', label: 'Payment retry failed', hint: 'A retry of a failed charge was declined.' },
  { key: 'notify_invoice_overdue', label: 'Invoice overdue', hint: 'An invoice passed its due date.' },
  { key: 'notify_credit_low', label: 'Credit balance low', hint: 'Projected usage is nearing your wallet balance.' },
  { key: 'notify_card_expiry', label: 'Card expired', hint: 'A saved card is no longer valid.' },
  { key: 'notify_mandate_reauth', label: 'Mandate re-authorisation', hint: 'A UPI Autopay mandate needs re-approval.' },
  { key: 'notify_trial_expiring', label: 'Trial ending', hint: 'A trial is about to end.' },
  { key: 'notify_payment_success', label: 'Payment received', hint: 'A charge succeeded (receipts).' },
]
...
const dirty = computed(() => {
  ...
  return PREFS.some(({ key }) => Boolean(Number(load.data![key] ?? 1)) !== state[key])
})
...
<template>
  <div class="mx-auto max-w-2xl">
    <p class="mb-4 text-p-sm text-ink-gray-5">
      Choose which billing events email you. Your in-app notifications always show every
      event regardless of these settings.
    </p>

    <div class="divide-y divide-outline-gray-1 rounded-lg ring-1 ring-outline-gray-1">
      <div
        v-for="pref in PREFS"
        :key="pref.key"
    ...
    <div v-if="canManageBilling" class="mt-4 flex justify-end">
      <Button variant="solid" :loading="save.loading" :disabled="!dirty" @click="onSave">
        Save preferences
      </Button>
    </div>
  </div>
