<template>
	...
	>
		...
		<div
			v-if="actions.length || $slots.action || $slots.footer || dismissible"
			class="-my-1 flex shrink-0 items-center gap-1 self-center"
		>
			<slot name="action" />
			<slot name="footer" />
			<button
