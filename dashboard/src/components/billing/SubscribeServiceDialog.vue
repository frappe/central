...
const selectedPlan = computed(() =>
	props.plans.find((p) => p.plan === plan.value),
)
...
const clusterOptions = computed(() =>
	(selectedPlan.value?.priced_clusters ?? []).map((c) => ({
		label: c,
		value: c,
	})),
)
...
watch(
	...
	() => {
		...
		cluster.value = priced.includes(cluster.value)
			? cluster.value
			: (priced[0] ?? '')
	},
	{ immediate: true },
...
function planLabel(p: ServicePlanOption): string {
	...
	const included = p.allowance
		? `, ${p.allowance.toLocaleString()} ${p.unit || 'unit'} included`
		: ''
