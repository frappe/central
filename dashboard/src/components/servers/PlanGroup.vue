...
<template>
	<div class="space-y-1.5">
		...
		<label
			...
			:key="plan.plan"
			class="flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2 text-sm transition-colors focus-within:border-outline-gray-4 focus-within:ring-1 focus-within:ring-outline-gray-4"
			:class="
		...
		<div
			v-if="profile"
			class="rounded-lg border transition-colors focus-within:border-outline-gray-4 focus-within:ring-1 focus-within:ring-outline-gray-4"
			:class="
				isCustom
					? 'border-outline-gray-4 bg-surface-gray-1'
					: 'border-outline-gray-2 hover:border-outline-gray-3'
		...
		>
			<label class="flex cursor-pointer items-center gap-3 px-3 py-2 text-sm">
				...
				<span class="shrink-0 font-medium text-ink-gray-9">{{
					isCustom ? customPrice : "Design your own"
				}}</span>
			</label>
				...
				<div class="overflow-hidden">
					<div class="border-t border-outline-gray-2 px-4 py-4">
						...
						<p v-if="matchingPreset" class="mt-3 text-p-xs text-ink-gray-5">
							...
							preset offers this exact shape — it may be cheaper than building it à
							la carte.
						</p>
					</div>
				</div>
