...
const initial = computed<ComposedConfig | null>(() =>
	resizable.value
		? {
				sub_category:
					configCall.data!.sub_category ?? profiles.value[0]?.sub_category ?? "",
           ...
           show a clear in-progress state and hold the dialog open until it lands. -->
			...
			>
				...
				<p class="max-w-xs text-p-sm text-ink-gray-5">
					...
					and comes back on its own when it’s done.
				</p>
			</div>
			<p v-else-if="configCall.loading || plansLoading" class="text-p-sm text-ink-gray-5">
				Loading…
			</p>
			...
			<div v-else class="space-y-4">
				...
				>
					Your server will be briefly stopped to apply the new size, then started again
					automatically.
				</div>
