...
async function submitPassword() {
	...
	error.value = "";
	if (emailError(email.value) || requiredError("Password")(password.value)) return;
...
async function runLogin(request: () => Promise<LoginResponse>) {
	...
	try {
		...
		window.location.replace(loginDestination(response, route.query["redirect-to"]));
	} catch (exception) {
...
function loginError(exception: unknown): string {
	const message = frappeErrorMessage(exception, "Unable to sign in. Please try again.");
...
function verificationPrompt(): string {
	...
	if (!verification?.setup) {
		return `Verification by ${
			verification?.method || "the configured method"
		} could not be sent. Contact your administrator.`;
	}
