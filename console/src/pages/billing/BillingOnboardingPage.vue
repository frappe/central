...
// opens another. Adding a step is a registry-only change (see onboarding/steps.ts).
//
...
const done = reactive<Record<string, boolean>>(
  Object.fromEntries(steps.map((s) => [s.key, false])),
)

// A step is locked until every preceding NON-optional step is complete.
function locked(i: number): boolean {
  return steps.slice(0, i).some((s) => !s.optional && !done[s.key])
}
function stateOf(i: number): 'complete' | 'locked' | 'active' {
...
const manualKey = ref<string | null | undefined>(undefined)
...
function onComplete(key: string, val: boolean): void {
  done[key] = val
}
...
function finish(): void {
  ...
  router.replace(typeof redirect === 'string' ? redirect : '/servers')
}
</script>
    ...
    <div class="mx-auto w-full max-w-2xl px-5 py-10">
      ...
      <div class="mt-6 flex items-center justify-between gap-3">
        <p class="text-p-sm text-ink-gray-5">
          ...
          <template v-else-if="requiredDone">
            Optional steps left — head to the dashboard whenever you're ready.
          </template>
