...
declare global {
  interface Window {
    ...
    paypal?: {
      Buttons: (config: Record<string, unknown>) => { render: (el: Element) => unknown }
    }
  }
}
...
export async function openRazorpayCheckout(
  order: GatewayOrder,
  { name = 'Central', description = '', displayPayPal = false }: RazorpayOptions = {},
  ...
  return new Promise<RazorpayHandles>((resolve, reject) => {
    ...
    if (displayPayPal) {
      options.config = {
        display: {
          blocks: { paypal: { name: 'Pay with PayPal', instruments: [{ method: 'paypal' }] } },
...
export async function mountPayPalButtons(
  ...
  return window.paypal
    .Buttons({
      ...
      onApprove: (data: { orderID?: string }) => onApprove?.(data.orderID || order.order_id || ''),
