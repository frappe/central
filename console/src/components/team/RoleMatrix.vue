        ...
        <tr class="bg-surface-elevation-1">
          ...
          >
            <div class="flex items-start justify-between gap-1">
              <div class="min-w-0">
                <p class="truncate text-sm font-semibold text-ink-gray-9">{{ role.role_name }}</p>
                <Badge v-if="!role.is_system" class="mt-1" theme="violet" label="Custom" />
              </div>
              <RoleRowActions
                :role="role"
        ...
        <template v-for="group in groups" :key="group.plane">
          ...
          >
            <td class="sticky left-0 z-10 bg-surface-elevation-1 px-4 py-2.5">
              ...
              <p class="mt-0.5 font-mono text-xs text-ink-gray-4">{{ cap.name }}</p>
            </td>
            <td
              v-for="role in roles"
              :key="role.name"
