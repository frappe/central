...
const roleOptions = computed(() =>
  roles.value.filter((r) => r.role_name !== 'Owner').map((r) => ({ label: r.role_name, value: r.name })),
)

const previewCaps = computed<string[]>(() => (role.value ? capsByRole.value[role.value] ?? [] : []))
...
type InviteParams = { team: string; email: string; role: string; expires_in_days: number }
const inviteCall = useCall<string, InviteParams>({ url: method(API.inviteTeamMember), method: 'POST', immediate: false })
