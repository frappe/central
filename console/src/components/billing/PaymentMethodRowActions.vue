...
const options = computed(() => {
  ...
  if (!props.method.is_default)
    items.push({ label: 'Make default', icon: 'lucide-star', onClick: () => emit('makeDefault', props.method) })
  if (!props.isFirst)
    items.push({ label: 'Move up', icon: 'lucide-arrow-up', onClick: () => emit('moveUp', props.method) })
  if (!props.isLast)
    items.push({ label: 'Move down', icon: 'lucide-arrow-down', onClick: () => emit('moveDown', props.method) })
  items.push({ label: 'Remove', icon: 'lucide-trash-2', onClick: () => emit('remove', props.method) })
  return items
})
</script>
