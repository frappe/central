...
def execute():
	...
	for profile in frappe.get_all("Billing Profile", filters={"currency": ["in", [None, ""]]}, pluck="name"):
		currency = (
			frappe.db.get_value("Price Lock", {"team": profile}, "currency") if price_lock_exists else None
		)
		if currency:
			frappe.db.set_value("Billing Profile", profile, "currency", currency, update_modified=False)
