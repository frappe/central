...
class TestServerResize(UnitTestCase):
	...
	def remote(self, state: str, cpu_millicores: int = 1000, memory_mib: int = 2048, disk_mib: int = 25600):
		return {
			"current_state": state,
			"compute": {"cpu_millicores": cpu_millicores, "memory_mib": memory_mib},
	...
	def test_compute_change_stops_resizes_grows_disk_and_starts(self):
		...
		self.client.update_compute.assert_called_once_with("vm-00001", 2000, 4096)
