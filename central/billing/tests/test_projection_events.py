...
class EventTestBase(IntegrationTestCase):
	...
	def _project(self, events_list, **kw):
		frappe.db.commit()
		return engine.project(TEAM, "2026-09-01", "2026-09-30", today=TODAY, events=events_list, **kw)
...
class TestTheChangeStreamIsInjectable(EventTestBase):
	...
	def test_an_injected_cancel_closes_the_segment(self):
		out = self._project(
			[{"event_type": events.CANCEL, "on_date": "2026-09-16 00:00:00", "subscription": self.sub}]
		)
	...
	def test_the_real_ledger_is_never_written(self):
		...
		self.assertEqual(frappe.db.count("Subscription Change", {"subscription": self.sub}), before)
...
class TestRefusals(EventTestBase):
	def test_a_provision_beyond_the_cap_is_reported_as_refused(self):
		...
		out = self._project(
			[
				{
					"event_type": events.PROVISION,
					"on_date": "2026-09-10 00:00:00",
					"subscription": self.sub,
					"plan": PLAN,
					"rate": 9000,
				}
...
class TestDeclines(EventTestBase):
	...
	def test_a_decline_does_not_move_any_date(self):
		...
		self.assertEqual([s["date"] for s in plain], [s["date"] for s in declined])

	def test_each_retry_says_which_method_it_would_reach_for(self):
		self._card("visa")
		self._card("mastercard")
		frappe.db.commit()
		out = self._project([{"event_type": events.DECLINE, "on_date": "2026-10-09 00:00:00", "attempt": 1}])
