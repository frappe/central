...
class TestEmandatePredebit(IntegrationTestCase):
	...
	def test_schedule_is_idempotent(self):
		...
		self.assertEqual(frappe.db.count(
			"Billing Notification Log", {"team": TEAM, "event_type": "Pre-debit Notice"}), 1)
	...
	def test_charge_due_only_after_window(self):
		...
		adapter = MagicMock()
		adapter.charge.return_value = PaymentResult(success=True, status="Captured", gateway_transaction_id="pay_e")
