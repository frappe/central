...
"""Billing Group dashboard endpoints (invoice-group partitioning, ARCHITECTURE.md §2.1).

A team creates its own Billing Groups and tags subscriptions into them to split
its bill — one invoice per group, plus the team's consolidated invoice for
everything left ungrouped. Team-scoped via the capability IAM like every
dashboard endpoint: reads need `billing:view`, mutations `billing:manage`. A
group is always resolved back to its own team before a mutation touches it, so
a team can never rename/disable another team's group by guessing its name.
...
retirement path, and Subscription.validate_billing_group already refuses new
tags onto a disabled group while generate.py folds its assets back into the
...
def create_billing_group(title: str, team: str | None = None) -> dict:
	...
	doc = frappe.get_doc(
		{"doctype": "Billing Group", "title": title, "team": team}
