...
def get_team_billing(team: str) -> dict:
	...
	return {
		...
		"invoices": frappe.get_all(
			"Invoice",
			filters={"team": team},
			fields=["name", "status", "total", "amount_paid", "currency", "period_end"],
			order_by="period_start desc",
		),
		"payment_attempts": frappe.get_all(
			"Payment Attempt",
			filters={"team": team},
			fields=["name", "status", "amount", "currency", "gateway", "failure_code", "resolved_by"],
...
def get_retention() -> dict:
	...
	for s in frappe.get_all("Subscription", fields=["team", "account_standing"]):
		...
		standing[s.team] = (
			s.account_standing
			if _STANDING_RANK.get(s.account_standing, 0) > _STANDING_RANK.get(cur, 0)
			else cur
		)
	total = len(standing)
...
def list_teams() -> list[dict]:
	...
	for s in _with_cluster(
		...
		t = teams.setdefault(
			s.team, {"team": s.team, "standing": "Current", "mrr": 0.0, "subscriptions": 0, "resources": 0}
		)
		rate = _plan_monthly_inr(s.plan, s.cluster)
	...
	for t in teams.values():
		...
		t["open_invoices"] = frappe.db.count(
			"Invoice", {"team": t["team"], "status": ["in", ["Open", "Overdue"]]}
		)
		t["invoices"] = frappe.db.count("Invoice", {"team": t["team"]})
		# Credit normalised to INR so the whole row reads on one (INR-equiv.) axis.
		t["credit_balance"] = frappe.utils.flt(
			_to_inr(credits.get_balance(t["team"])["balance"], currency), 2
		)
		t["currency"] = currency
		rows.append(t)
...
def get_delinquent_teams() -> list[dict]:
	...
	for s in frappe.get_all(
		"Subscription",
		filters=[["account_standing", "in", ["Past Due", "Suspended"]]],
		...
		overdue = frappe.get_all(
			"Invoice",
			filters={"team": s.team, "status": ["in", ["Open", "Overdue"]]},
			fields=["name", "status", "total", "amount_paid", "due_date"],
			order_by="due_date asc",
		)
		rows.append(
			{
				"team": s.team,
				"standing": s.account_standing,
				"outstanding": frappe.utils.flt(
					sum(frappe.utils.flt(i.total) - frappe.utils.flt(i.amount_paid) for i in overdue), 2
				),
				"invoices": overdue,
			}
		)
