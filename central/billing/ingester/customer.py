...
def create_customer_profile(billing_profile):
	...
	if customer:
		...
		gstin_details = get_gstin_details(billing_profile.gstin)
		billing_profile.db_set('profile_id', customer.name)
		billing_profile.db_set("gst_status", gstin_details.get('status'))
		build_tax_profile(billing_profile, gstin_details)
