...
_BILLING_EVENT_TYPES = {
	"payment_success": ("Payment received", "Payment received for invoice {{ reference_name }}.", "Success", "billing:view", None, None),
	"payment_failure": ("Payment failed", "Payment for invoice {{ reference_name }} failed: {{ message }}", "Error", "billing:view", "Pay now", "/billing/invoices"),
	"payment_retry": ("Payment retry failed", "Payment retry for invoice {{ reference_name }} failed: {{ message }}", "Warning", "billing:view", "Pay now", "/billing/invoices"),
	"invoice_overdue": ("Invoice overdue", "Invoice {{ reference_name }} is overdue. Please settle it to avoid suspension.", "Error", "billing:view", "Pay now", "/billing/invoices"),
	"credit_low": ("Credit balance low", "Your credit balance is low (projected use {{ message }}). Top up to avoid interruption.", "Warning", "billing:view", "Top up", "/billing"),
	"card_expiry": ("Card expired", "Your card has expired. Please add a new payment method.", "Warning", "billing:view", "Update card", "/billing"),
	"mandate_reauth": ("Mandate re-authorisation needed", "Your UPI Autopay mandate needs re-authorisation for the new limit.", "Warning", "billing:view", "Re-authorise", "/billing"),
	"trial_expiring": ("Trial ending", "Your trial is ending. Add a payment method to keep your resources running.", "Warning", "billing:view", "Add payment method", "/billing"),
	"action_required": ("Action required — choose how to pay", "Your usage is above the limit for automatic payments. Please choose to pay each invoice or prepay your wallet.", "Warning", "billing:view", "Choose how to pay", "/billing/invoices"),
	"pre_debit_notice": ("Upcoming auto-payment", "We'll auto-debit for your upcoming invoice. No action needed; this is a heads-up before the payment.", "Info", "billing:view", None, None),
	"team_suspension": ("Team suspended", "Your team has been suspended due to billing issues.", "Error", "billing:view", None, None),
}
...
def _render_log_body(slug: str, ref: str | None, msg: str | None) -> str:
	...
	event = frappe.db.get_value(
		"Notification Event Type", slug, "in_app_body", as_dict=False,
	)
	if not event:
		return msg or slug
