...
**Completed on 2026-09-23.** The dashboard now type-checks against a local declaration boundary for the pinned Frappe UI public API, and explicit Biome rules run locally and in CI. Server fetching, filters, navigation, commands, and synchronous SSO tab creation have one owner. The server page is smaller, and the resize dialog now renders state from one domain composable. Billing reports, invoice detail, spending limits, and metered service reads moved from pages and repeated cards into shared composables. Capability and Team Invitation lists now use Frappe UI `useList` with tested DocType list and document permissions, so two forwarding APIs are removed. Snapshot loaders clear their loading state when their scope disappears. `ServerMap` keeps its intentional viewport and animation state, and ListView keeps its existing pagination and state components. Action failures now use a persistent Frappe UI alert by default. Forms keep validation errors inline, while toasts are limited to short success feedback and non-actionable background status. Bucket, Cargo, and object-storage dashboard work remains deferred. Dashboard type-check, lint, and production build passed. The 38 focused IAM and team-management tests passed. A live browser smoke test covered server map and list actions, search, resize selection, billing reports, invoices, spending limits, services, team members, and the role capability palette. The test found and fixed a false resize change for the current preset and a wrong HTTP method for site login. The browser safety layer blocked the final authenticated server-tab navigation, so its request contract was verified from the route and client instead.
