...
export const test = base.extend({
	billing: async ({ page, request }, use) => {
		...
		async function login({ email, password }) {
			const res = await page.request.post(method("login"), {
		...
		const saveCard = ({ team, token } = {}) =>
			backend("save_test_card", { team, ...(token && { token }) });
		const makeInvoice = ({ team, total = 1180, linkCard = 0 }) =>
			backend("make_invoice", { team, total, link_card: linkCard });
		const settle = ({ team, invoice, collect = 1 }) =>
			backend("settle", { team, invoice, collect });
		...
		const setTrustTier = ({ team, maxSpend = 50000 }) =>
			backend("set_trust_tier", { team, max_spend: maxSpend });
		const setCollectionMode = ({ team, mode }) =>
			backend("set_collection_mode", { team, mode });
		...
		for (const c of seeded) {
			await request
				.post(method("central.billing.tests.e2e.teardown"), {
					form: { team: c.team, email: c.email },
				})
				.catch(() => {});
		}
	},
